...
const CreateNewGroupService = async (data) => {
    try {
        ...
        const {title_en, code, priority, users_uid, pages_uid, components_uid} = data
        const group = await new Group({uid: getTimestampInSeconds(), title_en, code, priority, users_uid, pages_uid, components_uid})
        const createdGroup = await group.save()

        if (!createdGroup) {
            return {status: false, message: customError.group.failed.add}
        } else {
            return {status: true, message: customError.group.success.add, groups: await group}
        }
    } catch (err) {
...
const UpdateGroupByIdService = async (req) => {
    try {
        ...
        if (!resUpdate) {
            return {status: false, message: customError.group.success.update}
        } else {
            return {status: true, message: customError.group.failed.update, group: await resUpdate}
        }
...
const DeleteGroupByIdService = async (id) => {
    try {
        ...
        if (!resDelete) {
            return {status: false, message: customError.group.success.delete}
        } else {
            return {status: true, message: customError.group.failed.delete, group: await resDelete}
        }
    } catch (err) {
        throw new Error(customError.server.error)
    }
